...
class Standings extends Component {
  ...
  render() {
    ...
    if (!_.isEmpty(teams)) {
      ...
      teamStandings = teamsInfo.map((t, index) => {
        return (
          <Table.Row>
            <Table.Cell selectable textAlign="right">
              <a href={`/team/${t.abbreviation}`}>
                {t.index + 1}. {t.teamName}
              </a>
            </Table.Cell>
    ...
    return (
      <Container>
        ...
        <Table sortable celled selectable striped>
          ...
          <Table.Body>{teamStandings}</Table.Body>
        </Table>
        <hr />
      </Container>
    );
