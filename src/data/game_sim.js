...
import {calculatePlayerOverall, calculateTeamOverall, calculateTeamDefence,
  calculatePlayersOverall, calculateTeamOffense, getTeamDefence, getTeamGoalies,
  getTeamWingers, calculatePlayersTeamwork, calculatePlayersAggression, calculatePlayersOffense} from '../data/stats';
...
export function simGame(team1, team2) {
  let t1Goals, t2Goals, t1Shots, t2Shots, t1PenaltyMinutes, t2PenaltyMinutes, stats = {}, shifts = [];
...
function simShift(t1Players, t2Players) {
  let stats = {};
  let t1Chances, t2Chances, gameLog = [], t1Goalie, t2Goalie;
  ...
  let t1PenaltyMinutes = 0, t2PenaltyMinutes = 0;
  //team
  //2,3,5 minutes
  let penaltyChance = 0;
  ...
  if(isPenalty){
    ...
    if(isT1Penalty){
      ...
      penaltyPlayer = t1Players[CHANCE.weighted(
        [0, 1, 2, 3, 4],
        [
      ...
      penaltyPlayer = t2Players[CHANCE.weighted(
        [0, 1, 2, 3, 4],
        [
  ...
  //CALCULATE SHOTS
  const isT1Advantage = diff !== 0
    ?
    t1LineOverall > t2LineOverall
    :
    _.random(0, 80) % 2 === 0;
  ...
  const t1OffDiff = calculateTeamOffense({players: t1Wingers}) - calculateTeamDefence({players: t2Defence});
  const t2OffDiff = calculateTeamOffense({players: t2Wingers}) - calculateTeamDefence({players: t1Defence});
  ...
  for (let i = 0; i < t1Chances; i++) {
    ...
    if(isGoal) {
      ...
      for (let j = 0; j < CHANCE.weighted([0, 1, 2], [20, 60, 30]); j++) {
        ...
        let assistPlayer = CHANCE.weighted(
          assistPlayers,
          passArray
        );
  ...
  function goalChance(shooter, goalie, offDiff) {
    let chance = 20 / (goalie.skills.goaltending + (calculatePlayerOverall(goalie) * 2));

    const shooterGoalieDiff = calculatePlayerOverall(shooter) - calculatePlayerOverall(goalie);
    ...
    //High or Low
    if(isHighShot){
      const highChance = (shooter.skills.shooting * _.random(0.95, 1.25)) - goalie.skills.high;
      if(highChance < -5){
        chance *= 0.9;
      }
      ...
      const lowChance = (shooter.skills.shooting * _.random(0.8, 1.15)) - goalie.skills.low;
...
export function simGamesToDate({schedule, endDate, startDate, teams}) {
  ...
  startDate = startDate || moment().year(1970).month(0).date(0).format();
  endDate = (moment(endDate).add(1, 'd')).format();
  ...
  for (let i = 0; i < gamesToSim.length; i++) {
    if(!_.isEmpty(gamesToSim[i])) {
      for (let j = 0; j < gamesToSim[i].games.length; j++) {
        ...
        teams[_.findIndex(teams, (t) => {return t.id = team1.id})] = simmedGame.team1;
        teams[_.findIndex(teams, (t) => {return t.id = team2.id})] = simmedGame.team2;
